...
| `project_message` | `cross_project_mailbox.enabled: true` | Sends an asynchronous, intent budgeted coordination note to another registered project. Supports `mode: "list"` probe mode, optional `category` field, and `launch_policy` behavior. Sends from both internal (TUI) and external (served) sessions. | OMO v4.10 |
| `project_note` | `cross_project_mailbox.enabled: true` | **Deprecated** - use `project_message`. Fire-and-forget note drop with no presence probe and no launch. Retained so existing callers keep working. | OMO v4.10 |
