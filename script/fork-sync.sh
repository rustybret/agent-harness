...
#
# Syncs this fork (rustybret/agent-harness) with upstream
# (code-yeongyu/oh-my-openagent) WITHOUT ever rebasing or force-pushing:
#
#   1. fetch upstream
#   2. fast-forward the pristine mirror branch (dev) to upstream/<branch>
...
#      the merge result
#   5. commit (--no-verify; the .githooks pre-commit regenerates
#      utils/models.json and would contaminate the sync commit) and push
#
...
#   FORK_SYNC_NO_PUSH=1 script/fork-sync.sh # skip all pushes (CI review mode)
#
# This script is the ONLY sanctioned sync path. It must never invoke
# history-rewriting or force-push commands; script/fork-sync-model.test.ts
# enforces that.
set -euo pipefail
...
echo "== merge $BRANCH into fork/local =="
...
if git merge --ff-only "$BRANCH" >/dev/null 2>&1; then
  if [[ "$(git rev-parse HEAD)" == "$(git rev-parse "$PRE_MERGE_HEAD")" ]]; then
    echo "fork/local already up to date with $BRANCH"
  else
    echo "fork/local fast-forwarded to $BRANCH"
  fi
else
