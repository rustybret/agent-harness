...
- \`aft_callgraph(op="impact")\`: Map all usages and blast radius before changes
- \`aft_refactor\`: Safe workspace-wide symbol moves and renames
...
- **\`aft_callgraph(op="impact")\`**: Map impact before changes - Refactoring
- **\`aft_refactor\`**: Safe workspace-wide symbol moves and renames - Refactoring
...
<tool_reference>
- \`aft_callgraph(op="impact")\` / \`aft_refactor\`: map impact and refactor safely — Refactoring.
- \`ast-grep\` skill / \`sg\` CLI: find structural patterns — Refactoring, Build.
- \`explore\` agent: codebase pattern discovery — Build, Research.
