  ...
  describe("#given two notes where B supersedes A from the same source", () => {
    describe("#when listUnread and drainUnread are called", () => {
      it("#then listUnread shows both but drainUnread picks only the latest", async () => {
        ...
        expect(await exists(path.join(inboxDir(), `${noteA.messageId}.md`))).toBe(false)
        expect(await exists(path.join(root, "coordination_notes", FROM_PROJECT_ID, "rejected", `${noteA.messageId}.md`))).toBe(true)
      })
    })
  })
  ...
  describe("#given B supersedes A in a LATER batch, after A was already delivered", () => {
    describe("#when drainUnread runs", () => {
      it("#then B is flagged as superseding a delivered note", async () => {
        ...
        expect(await exists(path.join(root, "coordination_notes", FROM_PROJECT_ID, "processed", "superseded", `${noteA.messageId}.md`))).toBe(true)
        expect(await exists(path.join(root, "coordination_notes", FROM_PROJECT_ID, "processed", "superseded", `${noteA.messageId}.superseded.json`))).toBe(true)
      })
    })
  })
