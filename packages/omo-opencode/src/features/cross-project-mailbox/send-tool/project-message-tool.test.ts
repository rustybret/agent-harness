...
function gateSpyDeps(config: CrossProjectMailboxConfig): GateSpyHandle {
...
describe("createProjectMessageTool - single send path (project_note superseded)", () => {
  it("delivers from an internal portless session instead of blocking with project_note guidance", async () => {
    // given
    await writeProjectMailboxConfig({ enabled: true, senders: PERMISSIVE_SENDERS })
    const handle = gateSpyDeps(cfg({ launch_policy: "disabled" }))
    const def = createProjectMessageTool(handle.deps)
  ...
  it("allows mode:list and returns the advisory budget table without sending", async () => {
    // given
    await writeProjectMailboxConfig({ enabled: true, senders: PERMISSIVE_SENDERS })
    const handle = gateSpyDeps(cfg())
    const def = createProjectMessageTool(handle.deps)
  ...
  it("sends with no detector wired at all (no mode plumbing on the send path)", async () => {
    // given
    await writeProjectMailboxConfig({ enabled: true, senders: PERMISSIVE_SENDERS })
    const def = createProjectMessageTool(realDeps(cfg()))
