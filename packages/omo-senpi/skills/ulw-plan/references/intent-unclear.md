...
<high_accuracy_auto>
Because the human did not steer, adversarial review SUBSTITUTES for the interview you skipped - this is what catches a bad default. Metis runs during plan generation as always; after Metis findings are folded and the plan file is complete, run the momus high-accuracy review defined in `full-workflow.md` AUTOMATICALLY - no "do you want a review?" question - and resubmit fresh until momus APPROVES, fixing every cited issue.
...
4. Metis folded -> auto momus review (fix cited gaps until it approves) -> brief LEADING with the approach and the defaults, surfaced in the human TL;DR for veto.
</worked_example>
