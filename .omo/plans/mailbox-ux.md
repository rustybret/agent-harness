...
**T11. First-registration toast (A TUI side)** — ✅ DONE (commit `87652a7c6`; `decideRegistrationToast` + legacy-senders flag-consume orchestration in `dialog/registration-notice.ts`, wired into `tui.ts`'s existing poll tick via `runRegistrationToastCheck`/`runLegacySendersNoticeCheck`; 16/16 new unit tests + 518/518 full mailbox suite + 5/5 tui.test.ts green, 0 LSP errors, clean workspace typecheck)
